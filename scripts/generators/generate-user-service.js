...
const writeText = (filePath, content, dryRun) => {
  if (dryRun) {
...
const withGeneratorSignature = (content, generatedFrom) => {
  const header = [
...
const upsertExportLine = (filePath, exportLine, dryRun) => {
...
const writeUserServicesRootIndex = (dryRun) => {
...
const makeViewIndexSource = (spec) => {
  return withGeneratorSignature(
...
const makeViewInputsSource = (spec) => {
  return withGeneratorSignature(
...
const ensureLocaleShape = (localeObj) => {
  if (!localeObj.UserServices) {
    localeObj.UserServices = {};
  }
};

const syncLocale = (spec, enLocale, arLocale) => {
